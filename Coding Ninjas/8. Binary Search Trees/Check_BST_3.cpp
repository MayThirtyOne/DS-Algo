...
bool checkBST3(BSTNode<int> *root, int mini = INT_MIN, int maxi = INT_MAX){
	if(root==NULL){
		return true;
	}
	...
	bool isleft = checkBST3(root->left, mini, root->data-1);
	bool isRight = checkBST3(root->right,root->data,maxi);
...
{ 
	...
	cout<<"BINARY TREE? "<<checkBST3(root)<<endl;
